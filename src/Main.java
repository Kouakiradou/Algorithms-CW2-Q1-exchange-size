...
public class Main
{
    ...
    public void run()
    {
        ...
        for (int size = 100000; size <= 10000000; size = size + 100000)
        {
            ...
            long qs_exchange = quicksorter.quicksort(qs_arr, 0, qs_arr.size() - 1);

            qs_numberSize.add(size);
            qs_timeUsed.add(qs_exchange);

            long hs_exchange = heapsorter.heapsort(hs_arr);
            hs_numberSize.add(size);
            hs_timeUsed.add(hs_exchange);
